...
export default function BoughtRewards({ logout }) {
    ...
    const getRewards = async () => {
        ...
        try {
            const response = await fetch('http://localhost:8080/rewards/bought', {
                method: "GET",
                headers: {
            ...
            if (parseRes.message === undefined) {
                setBoughtRewards(parseRes.bought_rewards);
            }
            else {
                setBoughtRewards([]);
            }
    ...
    return (
        <Grid align="center">
            <Grid container direction="row" align="center" justify="space-between">
                ...
                <Grid style={{ margin: "2%" }}>
                    <Grid align="right">
                        <Button component={Link} to="/rewards">Redeem Rewards</Button>
                    ...
                    <Table className={classes.table} aria-label="customized table">
                        <TableHead>
                            <TableRow>
                                ...
                                <StyledTableCell align="right">Reward Code</StyledTableCell>
                                <StyledTableCell align="right">Redemption Date</StyledTableCell>
                            </TableRow>
                        </TableHead>
                        <TableBody>
                            {boughtRewards
                                ...
                                .map((boughtReward) => {
                                    return (
                                        <StyledTableRow key={cnt++}>
                                            <StyledTableCell component="th" scope="row">
                                                {boughtReward.body}
                                            </StyledTableCell>
                                            <StyledTableCell align="right">{boughtReward.code}</StyledTableCell>
                                            <StyledTableCell align="right">{boughtReward.buy_date}</StyledTableCell>
                                        </StyledTableRow>
                                    );
                                })}
                        </TableBody>
